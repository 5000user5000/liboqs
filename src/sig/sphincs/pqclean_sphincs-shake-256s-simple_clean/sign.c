...
/*============================================================================
 * Multi-Layer Cache Support Implementation
 *============================================================================*/

/**
 * Initialize multi-layer cache.
 * For 256s, only 1 layer is practical.
 */
...
int crypto_sign_signature_multilayer_cached(uint8_t *sig, size_t *siglen,
    ...
    for (i = 0; i < SPX_D; i++) {
        ...
        /* Get tree index for cache lookup */
        tree_index = tree;

        /* Use cached signing for top layer if cache is available */
        if (cache && cache->initialized && i == SPX_D - 1) {
            merkle_sign_multilayer_cached(sig, root, &ctx, wots_addr, tree_addr,
                                           idx_leaf, tree_index, i, cache);
        } else {
            merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf);
        }
        sig += SPX_WOTS_BYTES + SPX_TREE_HEIGHT * SPX_N;
