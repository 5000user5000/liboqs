 ...
 * For 128s: only 1 layer supported (72 KB, ~14% speedup)
 */
...
int crypto_sign_signature_multilayer_cached(uint8_t *sig, size_t *siglen,
    ...
    for (i = 0; i < SPX_D; i++) {
        ...
        /* Use cached signing for top layer only (128s limitation) */
...
/**
 * Returns a signature using cached top layer (backward compatible).
 */
